    ...
    if(!empty($_POST['subtotal']) and !empty($_POST['neto'])){
        ...
        if($rr=mysql_fetch_array($ss)){
            ...
            if($cod_cliente=='cliente'){
                ...
                $nom_cliente=consultar('id','cliente',"id='$cod_cliente'");
            }
        ...
        while($rr=mysql_fetch_array($ss)){
            ...
            mysql_query("INSERT INTO factura_detalle (factura,cod,nom,cant,inv,iva,val,cat,fecha,hora,sucursal) 
            VALUES ('$id_maximo','$cod','$nom','$cant','$inv','$valor_iva','$val','$cat','$fecha','$hora','$id_sucursal')");
            ...
            if($inv=='s'){
             ...
             mysql_query("UPDATE producto SET status='OCUPADA', cliente_tmp='$nom_cliente', fech_entrega='$fech_entrega', fecha_in='$fecha_in' WHERE id='$cod' and control='s'");
        }
        if($mesa=='no'){
