            ...
            <div class="row">
                    <div class="col-sm-12">
                        <div class="card-box table-responsive">
                            <h4 class="m-t-0 header-title" align="center"><b>Reporte de Equipo por fecha</b></h4>                         
                             <?php 
                                    if(!empty($_POST['i'])){
                            ...
                            <div class="hidden">
                                      <hr/>
                                          <div style="font-size: 14px;"align="center">
                                             <strong>REPORTE DE KARDEX DE EQUIPOS</strong><br>                              
                                        </div> 
                                        <hr/>
                                    </div>
                            <table class="table table-striped table-bordered table-responsive" width="100%" style="font-size: 14px;"  border="0">
                                <thead>
                                    <tr>                                       
                                        <th><div align="center">Equipos y Servicios</div></th>
                                        <th><div align="center">Cantidad</div></th>
                                        <th><div align="center">Tipo D/H</div></th>
                                ...
                                <tbody>
                                    <?php
                                        $importe=0;$total=0; 
                                        $ss=mysql_query("SELECT SUM(val) as valx,cod,val FROM factura_detalle WHERE sucursal='$id_sucursal' and fecha between '$fechai' AND '$fechaf' group by cod ORDER BY val ASC");
                                        while($rr=mysql_fetch_array($ss)){
                                            $oHabitacion=new Consultar_Habitacion($rr['cod']);
                                            $cod_hab=$rr['cod'];
                                            $cod_hab=$rr['cod'];
                                            ###################################################################
                                            $sql =mysql_query("SELECT SUM(cant) as cant,cod FROM factura_detalle WHERE cod='$cod_hab' and fecha between '$fechai' AND '$fechaf'");
                                            if($rrx=mysql_fetch_array($sql)){
                                            }
                                             $importe=$rr['val']*$rrx['cant'];   
                                             $total=$total+$importe;

                                             if ($oHabitacion->consultar('control')=='s') {
                                                   $tipo='DIA';
                                               }
                                               elseif ($oHabitacion->consultar('control')=='n') {
                                                   $tipo='HORA';
                                               }
                                               else{
                                                $tipo='CANTIDAD';
                                    ...
                                    ?>
                                    <tr>                                       
                                        <td><?php echo $oHabitacion->consultar('nombre');  ?></td>
                                        <td align="center"><?php echo $rrx['cant']; ?></td>
                                        <td align="center"><?php echo $tipo;  ?></td>
                                        <td align="right"><strong><?php echo $s.formato($importe); ?></strong></td>                                    
                                    </tr>
                                     <?php } ?>
                                     <tr>
                                        <td colspan="3"><div align="right"><strong><h4>Total General</h4></strong></div></td>
                                        <td><div align="right"><strong><h4>$ <?php echo formato($total); ?></h4></strong></div></td>
                                    </tr>                                                               
                                </tbody>
                            </table>
                            </div>
